...
class MVTecDRAEM_Test_Visual_Dataset(Dataset):
    ...
    def __init__(
        ...
        self.images = sorted(glob.glob(root_dir + "/*/*.png"))[
            :2
        ]  # 獲取根目錄下所有子目錄中的 png 圖像，排序並只取前 2 張用於可視化測試。
    ...
    def transform_image(self, image_path, mask_path):
        ...
        if self.resize_shape is not None:  # 如果設置了調整大小的形狀。
            image = cv2.resize(
                image,  # 調整圖像大小。
                dsize=(self.resize_shape[1], self.resize_shape[0]),  # 目標寬度。
            )  # 目標高度。
            mask = cv2.resize(
                mask,  # 調整掩碼大小。
                dsize=(self.resize_shape[1], self.resize_shape[0]),  # 目標寬度。
        ...
        image = (
            np.array(image)
            .reshape(  # 將圖像轉換為 numpy 數組並調整形狀。
                (image.shape[0], image.shape[1], 3)
            )
            .astype(np.float32)
        )  # 確保是 3 通道且數據類型為 float32。
        mask = (
            np.array(mask)
            .reshape(  # 將掩碼轉換為 numpy 數組並調整形狀。
                (mask.shape[0], mask.shape[1], 1)
            )
            .astype(np.float32)
        )  # 確保是 1 通道且數據類型為 float32。

        image = np.transpose(
            image, (2, 0, 1)
        )  # 轉置圖像維度，從 (H, W, C) 變為 (C, H, W)，符合 PyTorch 格式。
        mask = np.transpose(
            mask, (2, 0, 1)
        )  # 轉置掩碼維度，從 (H, W, C) 變為 (C, H, W)。
    ...
    def __getitem__(self, idx):  # 定義 __getitem__ 方法，用於獲取單個樣本。
        ...
        if base_dir == "good":  # 如果是正常樣本。
            image, mask = self.transform_image(
                img_path, None
            )  # 調用轉換方法，掩碼為 None。
...
class MVTecDRAEMTrainDataset(Dataset):  # 定義訓練數據集類，用於 DRAEM 模型訓練。
    ...
    def augment_image(self, image, anomaly_source_path):
        ...
        perlin_scalex = 2 ** (
            torch.randint(
                min_perlin_scale, perlin_scale, (1,)  # 隨機生成 X 方向的 Perlin 尺度。
            ).numpy()[0]
        )
        perlin_scaley = 2 ** (
            torch.randint(
                min_perlin_scale, perlin_scale, (1,)  # 隨機生成 Y 方向的 Perlin 尺度。
            ).numpy()[0]
        )
        ...
        perlin_thr = np.where(
            perlin_noise > threshold,  # 對 Perlin 噪聲進行二值化，生成掩碼。
        ...
        if no_anomaly > 0.5:  # 50% 機率不添加異常。
            ...
            return (
                image,
                np.zeros_like(perlin_thr, dtype=np.float32),  # 返回原圖和全零掩碼。
                np.array([0.0], dtype=np.float32),  # 異常標籤為 0。
            )
        else:  # 50% 機率添加異常。
            augmented_image = augmented_image.astype(np.float32)  # 轉換增強後圖像類型。
            msk = (perlin_thr).astype(np.float32)  # 轉換掩碼類型。
            augmented_image = (
                msk * augmented_image + (1 - msk) * image
            )  # 確保異常區域覆蓋在原圖上。
    ...
    def transform_image(self, image_path, anomaly_source_path):
        ...
        augmented_image, anomaly_mask, has_anomaly = (
            self.augment_image(  # 調用 augment_image 生成合成異常圖像。
                image, anomaly_source_path
            )
        )
        augmented_image = np.transpose(
            augmented_image, (2, 0, 1)
        )  # 轉置增強圖像維度 (C, H, W)。
        image = np.transpose(image, (2, 0, 1))  # 轉置原圖像維度 (C, H, W)。
        anomaly_mask = np.transpose(
            anomaly_mask, (2, 0, 1)
        )  # 轉置異常掩碼維度 (C, H, W)。
    ...
    def __getitem__(self, idx):  # 定義 __getitem__ 方法。
        idx = torch.randint(
            0, len(self.image_paths), (1,)
        ).item()  # 隨機選擇一張正常圖像的索引。
        anomaly_source_idx = torch.randint(
            0, len(self.anomaly_source_paths), (1,)  # 隨機選擇一張異常源圖像的索引。
        ).item()
        image, augmented_image, anomaly_mask, has_anomaly = (
            self.transform_image(  # 轉換圖像並生成異常。
...
class MVTecDRAEMTestDataset(Dataset):
    ...
    def __init__(self, root_dir, category_name, resize_shape=None):  # 初始化函數。
        ...
        category_path = os.path.join(
            self.root_dir, self.category_name
        )  # 構建類別路徑。
        ...
        self.anomaly_masks_paths.extend(
            [None] * len(good_image_paths)  # 對應掩碼為 None。
        )  # 正常圖像沒有掩碼
        ...
        for defect_type in defect_types:  # 遍歷每種缺陷類型。
            ...
            self.is_anomaly_flags.extend(
                [1] * len(defect_image_paths)
            )  # 異常標記為 1。
            ...
            for img_path in defect_image_paths:  # 遍歷每張缺陷圖像。
                ...
                gt_mask_filename = (
                    f"{name_without_ext}_mask{ext}"  # 構建對應的掩碼文件名。
                )

                gt_mask_path = os.path.join(  # 構建完整的掩碼路徑。
                    ground_truth_path, defect_type, gt_mask_filename
                )  # 使用新的帶 _mask 的檔案名。
    ...
    def __getitem__(self, idx):  # 定義 __getitem__ 方法。
        ...
        anomaly_mask = np.expand_dims(anomaly_mask, axis=0)  # (H, W) -> (1, H, W)
        ...
        sample = {  # 構建返回樣本。
            ...
            "has_anomaly": np.array(
                [self.is_anomaly_flags[idx]], dtype=np.float32  # 異常標記。
            ),
            "idx": idx,  # 索引。
        }
