...
主要功能包括：
1. 載入訓練好的學生模型 (Student Model) 及其權重。
2. 定義 `predict_anomaly` 函數，用於對單張圖片進行異常檢測推論。
3. 執行影像預處理、模型前向傳播，並產生重建影像與異常遮罩。
4. 使用 Matplotlib 可視化原始影像、重建影像以及疊加的異常檢測結果。
"""

import torch  # 導入 PyTorch 庫，這是深度學習的主要框架。
from torchvision import (
    transforms,
)  # 導入 torchvision.transforms，用於圖像預處理和增強。
...
)  # 從 model_unet 模組導入 AnomalyDetectionModel 類，這是用於異常檢測的模型架構。
...
student_model = AnomalyDetectionModel(  # 創建 AnomalyDetectionModel 的實例。
    ...
    disc_in=IMG_CHANNELS
    * 2,  # 判別網絡輸入通道數（通常是原圖 + 重建圖的拼接，所以是 2 倍）。
...
# 載入訓練好的學生模型權重
MODEL_WEIGHTS_PATH = (
    "./student_model_checkpoints/student_model.pckl"  # 定義學生模型權重文件的路徑。
)
...
def predict_anomaly(
    ...
    )  # 應用預處理增加 batch 維度，並移動到設備。

    # --- 4. 執行前向傳播 (在 no_grad 上下文中以節省資源) ---
    with torch.no_grad():  # 禁用梯度計算，節省內存並加速推論。
        # 推論時，我們只需要分割圖，但模型會同時返回重建圖
        # 我們不需要特徵圖，所以 return_feats=False
        recon_image_tensor, seg_map_logits = model(
            image_tensor, return_feats=False
        )  # 執行模型前向傳播，獲取重建圖像和分割 logits。
    ...
    seg_map_probs = torch.softmax(
        seg_map_logits, dim=1
    )  # 對分割 logits 應用 Softmax，計算每個類別的概率。
    # 使用 argmax 找出每個像素點機率最高的類別，得到 [1, H, W] 的預測遮罩
    anomaly_mask_tensor = torch.argmax(
        seg_map_probs, dim=1
    )  # 取概率最大的類別索引作為預測結果。
    ...
    )  # 將原始圖像調整大小並轉換為 numpy 數組，用於顯示。

    # 反正規化重建圖像以便顯示
    recon_image_np = (
        recon_image_tensor.squeeze().cpu().numpy().transpose(1, 2, 0)
    )  # 將重建張量轉換為 numpy 數組，並調整維度順序為 (H, W, C)。
    ...
    anomaly_mask_np = (
        anomaly_mask_tensor.squeeze().cpu().numpy().astype(np.uint8)
    )  # 將異常掩碼轉換為 numpy 數組，並轉為 uint8 類型。

    return (
        original_image_np,
        recon_image_np,
        anomaly_mask_np,
    )  # 返回處理後的原始圖像、重建圖像和異常掩碼。


# --- 使用範例 ---
IMAGE_PATH_TO_TEST = (
    "path/to/your/test_image.png"  # 定義要測試的圖像路徑（請根據實際情況修改）。
)
original, reconstruction, anomaly_mask = predict_anomaly(
    student_model, IMAGE_PATH_TO_TEST, DEVICE
)  # 調用預測函數進行推論。

# --- 可視化結果 ---
fig, axes = plt.subplots(
    1, 3, figsize=(15, 5)
)  # 創建一個包含 3 個子圖的圖像，設置總大小。
...
axes[2].imshow(
    anomaly_mask, cmap="jet", alpha=0.4
)  # 將異常掩碼以半透明方式疊加在原圖上，使用 'jet' 顏色映射。
